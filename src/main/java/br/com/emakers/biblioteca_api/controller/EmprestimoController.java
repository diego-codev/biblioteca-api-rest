...
public class EmprestimoController {
    ...
    br.com.emakers.biblioteca_api.data.entity.Pessoa pessoa = (br.com.emakers.biblioteca_api.data.entity.Pessoa) authentication.getPrincipal();
        // Se USER, só pode acessar empréstimo próprio
        if (!pessoa.getRole().name().equals("ADMIN") && idPessoa != null && pessoa.getIdPessoa() != idPessoa) {
    ...
    emprestimoRequestDTO.setIdPessoa(pessoa.getIdPessoa());
        // Regra: limitar a 3 empréstimos ativos por usuário
        List<EmprestimoResponseDTO> ativos = emprestimoService.getEmprestimosAtivos();
    long qtdAtivos = ativos.stream().filter(e -> e.idPessoa().equals(pessoa.getIdPessoa())).count();
        if (qtdAtivos >= 3) {
    ...
    Long pessoaParaDevolver = pessoa.getRole().name().equals("ADMIN") && idPessoa != null ? idPessoa : pessoa.getIdPessoa();
    ...
    Long pessoaParaExcluir = pessoa.getRole().name().equals("ADMIN") && idPessoa != null ? idPessoa : pessoa.getIdPessoa();
    ...
    br.com.emakers.biblioteca_api.data.entity.Pessoa pessoa = (br.com.emakers.biblioteca_api.data.entity.Pessoa) authentication.getPrincipal();
        if (pessoa.getRole().name().equals("ADMIN")) {
            ...
            List<EmprestimoResponseDTO> meus = todos.stream().filter(e -> e.idPessoa().equals(pessoa.getIdPessoa())).collect(java.util.stream.Collectors.toList());
            if (meus.isEmpty()) {
